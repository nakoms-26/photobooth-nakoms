...
export default function CameraView({ onPhotosCaptured }: CameraViewProps) {
  ...
  const captureCurrentFrame = useCallback((applyFilter = true) => {
    ...
    if (!ctx) return null;
    ctx.save();
    if (isMirrored) { ctx.translate(width, 0); ctx.scale(-1, 1); }
    if (applyFilter) {
    ...
    <div className="relative w-full h-[100dvh] overflow-hidden bg-[var(--color-background)]">
      ...
      {cameraError ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-[var(--color-background)] z-0">
          ...
          <p className="text-[var(--color-text)] font-bold text-sm font-chillax text-center px-8">{cameraError}</p>
          <button onClick={startCamera} className="neo-btn-primary px-6 py-3 font-chillax font-bold">
            Coba Lagi
          </button>
        </div>
      ) : (
      ...
      {countdown !== null && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="font-chillax font-bold text-white text-2xl tracking-[0.3em] mt-2 drop-shadow-[2px_2px_0_#000]">
            BERPOSE!
          </span>
        </div>
      )}
      ...
      {capturePhase === 'REVIEW_ALL' && (
        <div className="absolute inset-0 z-20 pointer-events-none flex items-center justify-center">
          <div
            ...
            style={{ backgroundColor: 'var(--color-secondary)', boxShadow: '0 6px 0 #000' }}
          >
            <p className="font-black text-black text-lg">Semua foto siap!</p>
            <p className="text-black/60 text-xs mt-0.5">Klik thumbnail untuk mengulang</p>
          </div>
        </div>
      )}
      ...
      {(capturePhase === 'READY' || capturePhase === 'COUNTDOWN') && (
        <div className="absolute top-4 left-4 z-40">
          ...
          >
            FOTO {Math.min(currentPoseIndex + 1, TARGET_PHOTO_COUNT)}/{TARGET_PHOTO_COUNT}
          </div>
        </div>
      )}
      ...
      <div
        className="absolute bottom-0 left-0 right-0 z-40 flex flex-col items-center gap-4 pb-6 pt-12 px-4"
        style={{
        ...
        <div className="flex items-center justify-center gap-3">
          {Array.from({ length: TARGET_PHOTO_COUNT }).map((_, idx) => (
            <div
              key={idx}
              onClick={() => {
                if (capturePhase === 'REVIEW_ALL' && capturedPhotos[idx]) handleRetakeFromGrid(idx);
              }}
              ...
              `}
              style={{ backgroundColor: capturedPhotos[idx] ? 'transparent' : 'rgba(0,0,0,0.05)' }}
            >
              {capturedPhotos[idx] ? (
                <img src={capturedPhotos[idx]} className="w-full h-full object-cover" alt={`Foto ${idx + 1}`} />
              ) : (
                <div className="w-full h-full flex items-center justify-center">
                  <Camera className={`w-4 h-4 ${idx === currentPoseIndex ? 'text-red-500' : 'text-black/30'}`} />
                </div>
              )}
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-xl overflow-hidden border-2 transition-all ${
                      selectedFilter === f.id ? 'border-red-500 scale-110 shadow-sm' : 'border-black/20 hover:border-black/40'
                    }`}
                  >
                    {filterThumbnail ? (
                      <img
          ...
          {capturePhase === 'REVIEW' ? (
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                style={{ backgroundColor: 'var(--color-secondary)', boxShadow: '0 4px 0 #000' }}
              >
            ...
            <button
              ...
              style={{ backgroundColor: 'var(--color-secondary)', boxShadow: '0 5px 0 #000' }}
            >
            ...
            <button
              ...
              style={{ boxShadow: '0 0 0 3px rgba(255,255,255,0.2), 0 6px 20px rgba(0,0,0,0.5)' }}
            >
